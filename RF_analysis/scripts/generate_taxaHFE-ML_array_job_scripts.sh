#!/bin/bash
# =====================================================
# Usage: bash generate_array_job_scripts.sh
# Purpose: Automatically generate SLURM array scripts for all subsets/metabolite combinations 
# Note: This script assumes you've run 1) scripts/shared_subset_analysis_setup.sh and 2) scripts/generate_taxaHFE_ML_commands.sh already 
# =====================================================
...
SCRIPT_DIR="${BASE_DIR}/${subset}/microbiome/${metab}/scripts"
mkdir -p "${SCRIPT_DIR}"

# Commands file for this subset × metabolite
CMD_FILE="${BASE_DIR}/${subset}/microbiome/${metab}/commands/${subset}_${metab}_commands.txt"
...
STDOUT_DIR="${BASE_DIR}/${subset}/microbiome/${metab}/std_out"
mkdir -p "${STDOUT_DIR}"
...
#!/bin/bash
#SBATCH --job-name=${subset}_${metab}_microbiome
#SBATCH --output=${STDOUT_DIR}/${subset}_${metab}_microbiome_%A_%a.out
#SBATCH --error=${STDOUT_DIR}/${subset}_${metab}_microbiome_%A_%a.err
